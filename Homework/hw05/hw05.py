...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, temp_password):
        nonlocal tried_passwords, balance
        if len(tried_passwords) == 3:
            return "Too many incorrect attempts. Attempts: " + str(tried_passwords)

        elif temp_password != password:
            tried_passwords.append(temp_password)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    ret_value = withdraw(0, old_pass)

    if type(ret_value) == int:

        def joint(amount, temp_pass):
            if temp_pass == new_pass:
                return withdraw(amount, old_pass)
            else:
                return withdraw(amount, temp_pass)
