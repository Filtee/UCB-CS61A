...
def missing_digits(n):
    ...
    if n < 10:
        ...
        all_but_last, sec_last, last = n // 10, n // 10 % 10, n % 10

        if sec_last == last:
            return missing_digits(all_but_last)
        else:
            return missing_digits(all_but_last) + last - sec_last - 1
...
def count_change(total):
    ...
    def find_largest_coin(n=1):
        if n > total:
            return n // 2
        else:
            return find_largest_coin(n * 2)

    # find the partition possibilities
    def counting(n, m):
        if n == 0:
            return 1
        elif n < 0:
            ...
            return 0
        return counting(n - m, m) + counting(n, m // 2)

    # compute
    largest_coin = find_largest_coin()
    return counting(total, largest_coin)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: lambda n: f(f, n))(lambda f, n: 1 if not n else mul(n, f(f, sub(n, 1))))
