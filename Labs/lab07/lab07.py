def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + sub_s for sub_s in nested_list]
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = subseq_helper(s[1:], s[0])  # include the first element
            b = subseq_helper(s[1:], prev)  # exclude the first element
...
def reverse(lst):
    ...
    "*** YOUR CODE HERE ***"
    length = len(lst)
    for item in lst[:length]:
        lst.insert(length, item)
    del lst[:length]
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    full_grade, total_grade = 0, 0

    def student(str, grade):
        nonlocal full_grade, total_grade
        full_grade += class_assignments[str]
        total_grade += grade

        return total_grade / full_grade
