...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def main_lines():
            nearest_place = find_nearest_place(self.place, 0)
            if nearest_place:
                return rANTdom_else_none(nearest_place.bees)

        def find_nearest_place(current_place, current_range):
            # base cases
            if current_place is beehive or current_range > self.max_range:
                return None
            elif current_place.bees and self.min_range <= current_range <= self.max_range:
                return current_place
            # continue recursion
            return find_nearest_place(current_place.entrance, current_range+1)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # amount of damage bees receive
        bee_amount = amount
        if self.armor <= amount:
            bee_amount += self.damage
        # reduce armor of each bee
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return not (isinstance(other, ContainerAnt) or self.contained_ant)
        # END Problem 9
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        super().__init__(armor)
        if not self.hasQueen:
            self.trueQueen = True
            QueenAnt.hasQueen = True
        else:
    ...
    def action(self, gamestate):
        ...
        def main_lines():
            ...
            if self.trueQueen:
                ...
                # doubles damage of ants in her tunnel
                double_damage(self.place.exit)
            # if not trueQueen
            else:
        ...
        def double_damage(place):
            if place is None:
                ...
                if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant:
                    if not place.ant.contained_ant.has_doubled:
                        place.ant.contained_ant.damage *= 2
                        place.ant.contained_ant.has_doubled = True
            # continue recursion
            double_damage(place.exit)

        # execute the main_lines
        main_lines()
        # END Problem 13
